// This is the regular version of the assignment.
// Code is taken from exercise_3.cpp and exercise_4_basic.cpp and modified accordingly
...
void train_model(Net& model, const torch::Tensor& train_data, const torch::Tensor& train_target){
...
int compute_nb_errors(Net& model, const torch::Tensor input, const torch::Tensor target){
    ...
    for(int k = 0; k < input.size(0); k += mini_batch_size){  // Mini-batching loop
        ...
        for(int b = 0; b < mini_batch_size; ++b){
            if(target.index({k + b, predicted_class[b]}).item<float>() <= 0.0f){
...
auto main() -> int {
    ...
    train_model(model, train_data, train_target);  // Train
    int nb_test_errors = compute_nb_errors(model, test_data, test_target);  // Test
    
    std::cout << "Test Error: " << std::fixed << std::setprecision(2) 
    << (100.0 * nb_test_errors) / test_data.size(0) 
    << "% " << nb_test_errors << "/" << test_data.size(0) << std::endl;
